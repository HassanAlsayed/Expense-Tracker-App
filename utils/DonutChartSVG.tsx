...
const DonutChartSVG = () => {
 ...
 const { expenseSum, incomeSum } = sumOfValues();
  ...
  if (expenses.length === 0) {
    return (
      <View style={{flex:1, justifyContent:'center',alignItems: 'center'}}>
        <Text style={{fontSize:fn.getFontSize(20),color:'white'}}>No data available...</Text>
      </View>
    );
  }
  ...
  const colorList = expenses.map((item) => '#'+Math.floor(Math.random()*12344555).toString(16));
...
return (
  ...
  >
    <Text style={{color:'white',fontSize:fn.getFontSize(20),bottom:20}}>Total Balance: {incomeSum - expenseSum}$</Text>
    ...
    >
      {expenses.map((item, index) => (
        ...
        >
          ...
          />
          <Text style={{ color: 'white', fontSize: fn.getFontSize(8)}}>
            {item.name}
          </Text>
        </View>
      ))}
